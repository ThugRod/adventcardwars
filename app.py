...
def PersistVersion():
    ...
    data = {
        "maintenance_mode": "yes" if constants.GAME_IN_MAINTENANCE else "no",
...
def AdminPlayer(player):
    ...
    return render_template('admin_player.html', player=player, is_banned=IsUserBanned(player["username"]),
                           ...
                           InstalledDate=datetime.fromtimestamp(game["InstalledDate"], tz=timezone.utc).strftime(
                               '%Y-%m-%d %H:%M:%S GMT'), PVPBanned=bool(game["Zxcvbnm"]),
...
def AdminMaintenance():
    ...
    return render_template('admin_maintenance.html', maintenance=constants.GAME_IN_MAINTENANCE)
...
def AdminMaintenanceAction(action):
    ...
    if action == "enable":
        constants.GAME_IN_MAINTENANCE = True
    elif action == "disable":
        constants.GAME_IN_MAINTENANCE = False
    Log("admin", current_user.username + " updated maintenance mode to " + ("on" if constants.GAME_IN_MAINTENANCE else "off"))
...
def PersistMessagesGet(message):
    ...
    return send_from_directory(directory="", path=f"data/persist/messages/{message}.json", as_attachment=True,
...
def MultiplayerUpdateDeckName():
    ...
    if db_user.leader_level is None and int(clientData["leader_level"]) > 9999999:
        ...
        SystemBan(clientData["player_id"])
            ...
            db_user.leader_level) + 9999999:  # Make sure leader level is incremented by more than 10, if not, ban user
        DiscordWebhookMessage(
...
def AnalyticsPVPMatch():
    ...
    with open("data/persist/pvpmatches/" + headers.get("Player-Id", "unknown") + "_" + headers.get("Match-Id",
                                                                                                   "unknown") + ".json",
              "w") as outfile:
